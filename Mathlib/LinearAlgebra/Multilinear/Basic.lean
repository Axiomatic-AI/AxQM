...
@[simp]
theorem add_apply (m : ∀ i, M₁ i) : (g + g') m = g m + g' m :=
  rfl
...
@[simp]
theorem zero_apply (m : ∀ i, M₁ i) : (0 : MultilinearMap σ M₁ M₂) m = 0 :=
  rfl
...
instance : SMul S (MultilinearMap σ M₁ M₂) :=
  ⟨fun c f =>
    ⟨fun m => c • f m, fun m i x y => by simp [smul_add],
      fun l i x d => by
        change c • f (update l i (x • d)) = σ x • c • f (update l i d)
        rw [f.map_update_smulₛₗ, (smul_comm (σ x) c (f (update l i d))).symm]⟩⟩

@[simp]
theorem smul_apply (f : MultilinearMap σ M₁ M₂) (c : S) (m : ∀ i, M₁ i) : (c • f) m = c • f m :=
  rfl

theorem coe_smul (c : S) (f : MultilinearMap σ M₁ M₂) : ⇑(c • f) = c • (⇑f) := rfl
...
@[simps] def coeAddMonoidHom : MultilinearMap σ M₁ M₂ →+ (((i : ι) → M₁ i) → M₂) where
  toFun := DFunLike.coe; map_zero' := rfl; map_add' _ _ := rfl

@[simp]
theorem coe_sum {α : Type*} (f : α → MultilinearMap σ M₁ M₂) (s : Finset α) :
    ⇑(∑ a ∈ s, f a) = ∑ a ∈ s, ⇑(f a) :=
  map_sum coeAddMonoidHom f s

theorem sum_apply {α : Type*} (f : α → MultilinearMap σ M₁ M₂) (m : ∀ i, M₁ i) {s : Finset α} :
    (∑ a ∈ s, f a) m = ∑ a ∈ s, f a m := by
  classical
  induction s using Finset.induction with
  | empty => rfl
  | insert _ _ h ih => simp [Finset.sum_insert h, ih]
...
instance [Semiring R₂] {σ : R →+* R₂} [Module R₂ M₂]
    ...
    DistribMulAction S (MultilinearMap σ M₁ M₂) := fast_instance%
...
instance instModule [Semiring R₂] {σ : R →+* R₂} [Module R₂ M₂]
    [Module S M₂] [SMulCommClass R₂ S M₂] :
    Module S (MultilinearMap σ M₁ M₂) := fast_instance%
  coe_injective.module _ coeAddMonoidHom fun _ _ ↦ rfl

instance [Semiring R₂] {σ : R →+* R₂} [Module R₂ M₂] [Module S M₂] [SMulCommClass R₂ S M₂]
...
instance : AddCommGroup (MultilinearMap σ M₁ M₂) := fast_instance%
